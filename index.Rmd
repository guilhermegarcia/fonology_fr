...
<img src="cat.gif" alt="pipe" style="width:30%;">
...
```{r, echo = TRUE}
ipa(word = c("Encontramos", "transcrição", "fonética", "fina", "também"), narrow = T)
```
...
.pull-right[
- <mark>`biGramTbl()`</mark> $\rightarrow$ all bigrams
- Now it's easy to plot patterns


### Visualizing bigrams

- Together with `biGramTbl()`:
- <mark>`plotBiGrams()`</mark> $\rightarrow$ figures with `ggplot2`
...
lus_bigramas |> 
  plotBiGrams(type = "lollipop", n = 10) #<<
```
]
...
```{r, fig.align='center', fig.width=5, fig.height=3, fig.retina=TRUE, dpi = 800, out.height="300px", echo = TRUE}
ipa("obrigado") |> 
  plotSon(syl = T)
```
